...
import { pickCurrentSalary } from "../../validators/salaries.js";
...
export class MemoryEmployeeRepository implements EmployeeRepository {
  ...
  async getById(employeeId: string) {
    return this.rows.find((row) => row.id === employeeId) ?? null;
  }

  private findByEmployeeCode(code: string) {
  ...
  async insert(employee: Employee) {
    ...
    if (this.findByEmployeeCode(next)) {
  ...
  async update(employee: Employee) {
    const other = this.findByEmail(employee.email);
...
export class MemorySalaryRepository implements SalaryRepository {
  ...
  async getCurrentForMany(employeeIds: string[], today: string) {
    ...
    for (const employeeId of employeeIds) {
      const current = pickCurrentSalary(
        this.rows.filter((row) => row.employeeId === employeeId),
        today,
      );
      if (current) {
        latest.set(employeeId, current);
      }
    }
    return latest;
  }
