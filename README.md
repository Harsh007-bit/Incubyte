...
4. Mark someone inactive. History is still there. Insights drop them from spend.
...
hire-without-salary. Integration tests run only when `DATABASE_URL` or
`TEST_DATABASE_URL` is set — those prove UNIQUE / CHECK in Postgres.
